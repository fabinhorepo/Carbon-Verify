...
type SortField = 'name' | 'project_type' | 'country' | 'registry' | 'grade' | 'score' | 'credits' | 'alerts';
...
export default function Projects() {
  ...
  const getSortIcon = (field: SortField) => {
    ...
    return sortDir === 'asc' ? <ArrowUp className="w-3.5 h-3.5 text-emerald-600" /> : <ArrowDown className="w-3.5 h-3.5 text-emerald-600" />;
  };
  ...
  const sortedProjects = [...projects].sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case 'registry': return dir * (a.registry || '').localeCompare(b.registry || '');
      case 'grade': {
        const ga = GRADE_ORDER[a.rating?.grade] || 99;
        const gb = GRADE_ORDER[b.rating?.grade] || 99;
        return dir * (ga - gb);
      }
      case 'score': return dir * ((a.rating?.overall_score || 0) - (b.rating?.overall_score || 0));
      case 'credits': return dir * ((a.total_credits_issued || 0) - (b.total_credits_issued || 0));
      case 'alerts': return dir * ((a.fraud_alert_count || 0) - (b.fraud_alert_count || 0));
      default: return 0;
    }
  });
  ...
  const SortableHeader = ({ field, label, align = 'left' }: { field: SortField; label: string; align?: string }) => (
    <th
      className={`py-3 px-4 font-semibold text-slate-600 cursor-pointer hover:bg-slate-100 select-none transition-colors text-${align}`}
      ...
      {sortField && (
        <div className="flex items-center gap-2 text-sm text-slate-500">
          ...
          <span className="font-medium text-emerald-600">
            {{
              name: 'Projeto', project_type: 'Tipo', country: 'País', registry: 'Registro',
              grade: 'Rating', score: 'Score', credits: 'Créditos', alerts: 'Alertas'
            }[sortField]}
          </span>
          <span>({sortDir === 'asc' ? 'crescente' : 'decrescente'})</span>
          <button onClick={() => { setSortField(null); setSortDir('asc'); }}
            className="text-xs text-red-500 hover:text-red-700 ml-1">Limpar</button>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <SortableHeader field="name" label="Projeto" />
                <SortableHeader field="project_type" label="Tipo" />
                <SortableHeader field="country" label="País" />
                <SortableHeader field="registry" label="Registro" />
                <SortableHeader field="grade" label="Rating" align="center" />
                <SortableHeader field="score" label="Score" align="center" />
                <SortableHeader field="credits" label="Créditos" align="center" />
                <SortableHeader field="alerts" label="Alertas" align="center" />
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between px-4 py-3 border-t border-slate-200 bg-slate-50 gap-3">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => goToPage(1)} disabled={page === 1}
                className="p-1.5 rounded-md border border-slate-300 text-slate-500 hover:bg-white hover:text-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" title="Primeira página">
                <ChevronsLeft className="w-4 h-4" />
              </button>
              <button onClick={() => goToPage(page - 1)} disabled={page === 1}
                className="p-1.5 rounded-md border border-slate-300 text-slate-500 hover:bg-white hover:text-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" title="Página anterior">
                  ...
                  typeof p === 'string' ? (
                    ...
                    <button key={p} onClick={() => goToPage(p as number)}
                      className={`min-w-[36px] h-9 rounded-md border text-sm font-medium transition-colors ${
                        page === p ? 'bg-emerald-600 border-emerald-600 text-white' : 'border-slate-300 text-slate-600 hover:bg-white hover:text-slate-800'
                      }`}>
                      {p}
                    </button>
                  )
              ...
              <button onClick={() => goToPage(page + 1)} disabled={page === totalPages}
                className="p-1.5 rounded-md border border-slate-300 text-slate-500 hover:bg-white hover:text-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" title="Próxima página">
                <ChevronRight className="w-4 h-4" />
              </button>
              <button onClick={() => goToPage(totalPages)} disabled={page === totalPages}
                className="p-1.5 rounded-md border border-slate-300 text-slate-500 hover:bg-white hover:text-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" title="Última página">
